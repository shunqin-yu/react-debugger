...
function getRollupOutputOptions(
  ...
  return {
    ...
    sourcemapPathTransform(relativeSourcePath) {
      return relativeSourcePath.replace('../../../../packages', '/Users/ysq/Desktop/react-debugger/public/react/packages')
    },
  };
}
...
// reform
function copyFile2Public() {
  ['react', 'react-dom'].forEach(name => {
    const orgPath = `build/node_modules/${name}/umd/${name}.development.js`
    const desPath = `${path.resolve('../umd')}/${name}.development.js`

    fs.copyFileSync(orgPath, desPath)
    fs.copyFileSync(`${orgPath}.map`, `${desPath}.map`)
  })
}
